...
### ♻️ 代码重构
- **商品详情页优化**: 
  - **样式分离**: 将内联 CSS 提取至 `public/css/detail.css`
  - **逻辑分离**: 将内联 JavaScript 提取至 `public/js/detail.js`
  - **模板简化**: 大幅精简 `views/detail.ejs` 结构，提升代码可维护性

### 🐛 问题修复
- **模板错误修复**: 修复了详情页库存显示逻辑中的 EJS 语法错误（`<%=` 标签未闭合），解决了 500 Server Error 问题
